...
t1b = 
  ...
  summarise(var="Notifications included (1 March, 2020 to 4 February, 2021)",
            n_pop=sum(n_pop)/2,
            n_pos=sum(n_pos),
...
t1c = 
  ...
  summarise(var="  included (23 May, 2020 to 4 February, 2021)",
            n_pop=sum(n_pop)/2,
...
tmp_icudeaths  = strat_covid_sep_period8june %>%
  mutate(age_group2=case_when(age_group=="0-9"~"40-49",
                              age_group=="10-19"~"40-49",
                              age_group=="20-29"~"40-49",
                              age_group=="30-39"~"40-49",
                              age_group=="40-49"~"40-49",
                              age_group %in% c("0-49","50-59","60-69","70-79","80+") ~age_group
  )) %>%
  group_by(var=age_group2) %>%
  summarise(n_icu=sum(n_icu),
            n_death=sum(n_death)) %>%
  mutate(n_icu=paste0(fsep(n_icu),fperc(n_icu,sum(n_icu))),
         n_death=paste0(fsep(n_death),fperc(n_death,sum(n_death))))
t1d$n_test = tmp_test$n_test
t1d$n_icu[5:9] = tmp_icudeaths$n_icu
t1d$n_death[5:9] = tmp_icudeaths$n_death
