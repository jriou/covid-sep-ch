...
crude_model_estimates_8june = NULL
crude_model_predictions_8june = NULL
for(j in 1:length(model_names)) {
  ...
  # concatenate
  crude_model_estimates_8june = bind_rows(crude_model_estimates_8june,r_estimates)
  crude_model_predictions_8june = bind_rows(crude_model_predictions_8june,r_pred)
  cat("\n",j)
}
...
adjusted_model_estimates_8june = NULL
adjusted_model_predictions_8june = NULL
for(j in 1:length(model_names)) {
  m_outcome = case_when(grepl("m_test_",model_names[j]) ~ 1,
                        grepl("m_pos_",model_names[j]) ~ 2,
                        grepl("m_hospit_",model_names[j]) ~ 3,
  ...
  r_pred = posterior_predict(get(model_names[j]),
    ...
    mutate(model_name = model_names[j],
           ...
           n_pop = n_pop/length(unique(r_data$period)))
  # concatenate
  adjusted_model_estimates_8june = bind_rows(adjusted_model_estimates_8june,r_estimates)
  adjusted_model_predictions_8june = bind_rows(adjusted_model_predictions_8june,r_pred)
  cat("\n",j)
}
...
interaction_model_estimates_8june = NULL
interaction_model_predictions_8june = NULL
for(j in 1:length(model_names)) {
  m_outcome = case_when(grepl("m_test_",model_names[j]) ~ 1,
                        grepl("m_pos_",model_names[j]) ~ 2,
                        grepl("m_hospit_",model_names[j]) ~ 3,
  ...
  r_pred = posterior_predict(get(model_names[j]),
    ...
    mutate(model_name = model_names[j],
           ...
           n_pop = n_pop/length(unique(r_data$period)))
  # concatenate
  interaction_model_estimates_8june = bind_rows(interaction_model_estimates_8june,r_estimates)
  interaction_model_predictions_8june = bind_rows(interaction_model_predictions_8june,r_pred)
  cat("\n",j)
}
