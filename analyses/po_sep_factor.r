...
table(samples_estimates$par)


tmp_est = 
  samples_estimates %>%
  filter(grepl("factor\\(ssep_d\\)",par)) %>%
  ...
  mutate(period=if_else(data_type=="strat_covid_sep_period8june","31 March, 2020 to 14 April, 2021","23 May, 2020 to 14 April, 2021")) %>%
  mutate(period=factor(period,levels=c("31 March, 2020 to 14 April, 2021","23 May, 2020 to 14 April, 2021"))) %>%
  mutate(model_type=factor(model_type,levels=c("Adjusted","Crude"),labels=c("Adjusted","Unadjusted"))) %>%
  separate(par,into=c("par1","par2","par3","ssep_n")) %>%
  select(-par1,-par2,-par3) %>%
  mutate(ssep_n=as.numeric(ssep_n))

# choice 1 ----

g_est = ggplot(tmp_est) +
  geom_hline(yintercept=1,linetype=2,colour="grey30") +
  geom_pointrange(aes(x=ssep_n,y=RR,ymin=lb,ymax=ub,colour=outcome_name,shape=period),
                  size=.5) +
  ...
  labs(x="SEP group",y="Incidence rate ratio (ref. group 1)",shape="Data collection") +
  coord_flip() +
...
## save
ggsave(plot = grid.draw(g_est_grob),
       file="figures/suppfigure_sep_factor.pdf",
